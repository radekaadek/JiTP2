...
MenuWindow::MenuWindow(Graph_lib::Point loc, int w, int h, const std::string &label) : Graph_lib::Window(loc, w, h, label),
...
void actionDescriptor::cb_item(Graph_lib::Address, Graph_lib::Address pAD)
{
...
void MenuItem::attach(MenuWindow *pWnd, MenuHeader *pMenu, Graph_lib::Color color, actionDescriptor::Action act)
{
    ...
    pWnd->attach(*this);
    pw->callback(reinterpret_cast<Fl_Callback *>(actionDescriptor::cb_item), &buttonAction);
}
...
void MenuHeader::attach(MenuWindow *pWnd, const std::vector<colorSpec> &colors, actionDescriptor::Action act)
{
    ...
    pw->callback(reinterpret_cast<Fl_Callback *>(actionDescriptor::cb_item), &mAction);
}
